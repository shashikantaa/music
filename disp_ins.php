...
<?php } ?>
      ...
      <h3 style="text-align:left;color:white">INSTRUMENTALS</h3>
      <!--SONGS DISPLAY-->
      <?php
    @$conn=@mysqli_connect("localhost","root","","db_project");
    @$ins="SELECT * FROM tb_ins";
...
<div >
  <table align="center" width="100%"  style="background-color:black; border-color:white; border-top:none; border-left:none; border-right:none; border-top-color:black; border-right-color:black; border-left-color:black;">
        <tr class="song_disp">
            <td>
            <a  class="song_link" href="ins_play.php?id=<?php echo $value['ins_id']?>&name=<?php echo $value['ins_name'];?>&l=<?php echo $value['ins_loc'];?>"><?php echo $value['ins_name'];?></a>
            </td>
        </tr>
    </table>
